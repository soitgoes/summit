module.exports = function (Handlebars, _) {
  var dgFuncs = function dgFuncs() {
    $(function () {
      $('.select-all').change(function () {
        var checked = $(this).is(':checked');
        $($(this).parents('table').get(0)).find('.id-checkbox').prop('checked', checked);
      });
    });
  }
  ...
  Handlebars.registerHelper('deltaGrid', function () {
    ...
    var url = this.url || '';
    var info = this.info;
    var rowClasses = this.rowClasses;
    var noResultsMessage = this.noResultsMessage;
    var selectAll = this.selectAll;
    var scriptBlock = this.scriptBlock || 'scripts';
    ...
    var html = '<style>@media (max-width: 383px) { .btn-xxs-block { width: 100%; margin-bottom: 5px; } }</style>';
    ...
    _.forIn(actions, function (action, key) {
      if ((list && list.length) || action.showOnEmpty) {
        if (action.html)
          ...
          html += '<a href="' + action.href + '" class="btn btn-default btn-sm btn-xxs-block ' + (action.classes || '') + '">' + Handlebars.Utils.escapeExpression(key) + '</a> ';
        else
          html += '<input type="submit" class="action btn btn-default btn-sm btn-xxs-block ' + (action.classes || '') + '" name="action" value="' + Handlebars.Utils.escapeExpression(key) + '" /> ';
      }
    });
    html += '</div></div></div>';
    ...
    if (!list || !list.length) {
      ...
      list.forEach(function (item) {
        if (rowClasses && rowClasses.classes) {
          if (rowClasses.predicate && rowClasses.predicate(item, list))
            html += '<tr class="' + rowClasses.classes + '">';
          else if (!rowClasses.predicate)
            html += '<tr class="' + rowClasses.classes + '">';
          else
            html += '<tr>'
        }else{
        ...
        _.forIn(columns, function (c, key) {
          ...
          if (c.formatter) {
            ...
            html += '<input type="checkbox" class="id-checkbox" name="' + key + '" value="' + item[key] + '" />';
          else
    ...
    if (Handlebars.helpers['content']) {
      Handlebars.helpers['content'].call(this, scriptBlock, _.merge({ 
        hash: { mode: 'append' }, 
        fn: function () {
          html = '<script type="text/javascript">\n';
          html += dgFuncs.toString();
          html += '\ndgFuncs();\n</script>';
