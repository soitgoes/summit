...
function Summit (userConfig) {
  ...
  this.inject('_', _);
  this.inject('Handlebars', Handlebars);
  ...
  console.log(this.config.mailerOptions)
  this.inject('Mailer', nodemailer.createTransport(this.config.mailerOptions));
  ...
  this.invoke(require('./helpers/delta-grid'));
  ...
  this.use(this._middleware.poweredBy);

  mailer(this, Handlebars);
  forgotPassword(this);
}
...
Summit.prototype.userCollections = function() {
  return _.filter(_.values(this.collections), {isUserType: true});
};
...
Summit.prototype._setup = function () {
  ...
  var hbsMiddleware = hbs(this.viewDirs, Handlebars)
  ...
  this.use(bogart.middleware.gzip());
...
module.exports = function () {
  ...
  Summit.bogart = bogart;
  Summit.Collection = Collection;
  Summit.uuid = uuid;
  Summit.Facebook = FB;
  Summit.allow = allow;
  Summit.hash = hash;
  ...
  Summit.app = function (config) {
    return new Summit(config || {});
  }
