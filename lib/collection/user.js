...
UserCollection.prototype.findByUsername = function (username, opts) {
  opts = opts || {stale:"update_after"};
  opts.key = username;
  return this.view('byUsername', opts)
...
UserCollection.prototype.findByEmail = function (email, opts) {
  ...
  return this.view('byEmail', params, opts || {stale:"update_after"})
  .then(function (results) {
    if (results.length === 0) {
      return false;
    }
...
UserCollection.prototype.validate = function (user) {
  ...
  .then(function (results) {
    ...
    if (results[0].rows.length !== 0) {
      conflictingKey = _.map(results[0].rows, 'key')[0].split(':');
      conflictingUserId = _.map(results[0].rows, 'value')[0];
